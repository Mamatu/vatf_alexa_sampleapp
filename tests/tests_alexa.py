...
from vatf.api import audio, player, wait, shell, log_snapshot, sampling
from vatf.utils import config_handler
from vatf.utils import libtools
from vatf.utils import wait_types as w_t

def setup_function():
    config_handler.init_configs(["tests/config/test_config.json"])
...
def teardown_function():
    ...
    shell.fg(f"cd {config.sampling.path_to_tools} && sh convert_all_pcm_to_ogg.sh", stderr_exception = False)
    sampling.start_from_config()
...
def test_2():
    ...
    play_audio("alexa_tell_me_a_joke.wav")
